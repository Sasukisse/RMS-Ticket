...
function log_admin_action($action, $details = '', $user_id = null) {
    ...
    // La colonne `details` a une contrainte json_valid — on encode systématiquement en JSON
    $details_json = json_encode($details, JSON_UNESCAPED_UNICODE);

    $sql = 'INSERT INTO admin_logs (user_id, action, entity_type, details, ip_address, user_agent, created_at)
            VALUES (?, ?, \'\', ?, ?, ?, NOW())';
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="log-details">
                                        <?= e(json_decode($log['details'], true) ?? $log['details']) ?>
                                    </div>
                                </td>
                                <td>
